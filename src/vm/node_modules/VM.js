 ...
 *
 */
function createHostConfFileMounts(vmobj, log, callback) {
    ...
    // for docker links we might need to add additional hosts here.
...
function buildZonecfgUpdate(vmobj, payload, log)
{
    ...
    /*
     * zlog-mode should always be set for 'docker' VMs. If docker:tty is true
     * it should be set to 'interactive', in all other cases it should be set
     * to 'logging'.
     */
    ...
    if (payload.hasOwnProperty('add_nics')) {
        for (n in payload.add_nics) {
            if (payload.add_nics.hasOwnProperty(n)) {
                ...
                if (!nic.hasOwnProperty('mac')) {
                    nic.mac = nic.hasOwnProperty('vrrp_vrid') ? vrrpMAC(nic.vrrp_vrid)
                            : generateMAC();
                }
                delete nic.index;
                if (nic.hasOwnProperty('interface')) {
...
function doDockerStop(vmobj, options, callback)
{
    ...
    zonecfg(['-u', vmobj.uuid, unset_autoboot], log,
        ...
        // if init has died but zone is still running, pid is reported by mdb to
